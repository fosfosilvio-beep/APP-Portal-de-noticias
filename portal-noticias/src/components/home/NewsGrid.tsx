...
export default function NewsGrid({ title, icon, news, limit = 18, paginated = false }: NewsGridProps) {
  ...
  // Se não for paginado, respeita o limite passado ou o padrão
  const effectiveLimit = paginated ? visibleCount : limit;
  const displayNews = news.slice(0, effectiveLimit);
  const hasMore = news.length > effectiveLimit;
  ...
  return (
    <section className="flex flex-col mt-4">
      {/* Cabeçalho Limpo (Sem botão Ver Mais no topo) */}
      ...
      <div className="flex flex-col divide-y divide-slate-200">
        {displayNews.map((noticia, index) => (
          <div key={noticia.id}>
            ...
            {/* Divisor / Rodapé após cada bloco de 8 notícias (exceto se for a última da lista visível e não houver mais) */}
            {paginated && (index + 1) % 8 === 0 && (index + 1) < news.length && (
              <div className="py-12 flex flex-col items-center">
                ...
                {/* Só mostra o botão se este bloco for o fim da lista visível atual */}
                {(index + 1) === visibleCount && hasMore && (
                  <button 
                    onClick={loadMore}
      ...
      {/* Caso o total não seja múltiplo de 8, mostramos o botão final se houver mais */}
